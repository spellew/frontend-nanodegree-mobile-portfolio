...
function changePizzaSizes(size, el) {
  el.className = "randomPizzaContainer";
  el.classList.add(size.toLowerCase());
}

function removeSizeClasses(el, cl) {
  cl !== "randomPizzaContainer" ? el.classList.remove(cl) : null;
}
...
var resizePizzas = function (size) {
  ...
  randomPizzaContainers.forEach(changePizzaSizes.bind(null, size));
...
function changeLeft(el, i) {
  var phase = Math.sin((scrollTop / 1250) + (i % 5));
  el.style.left = el.basicLeft + 100 * phase + 'px';
}
...
function updatePositions(scrollTop) {
  ...
  Array.from(document.getElementsByClassName("mover")).forEach(changeLeft);
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  window.randomPizzaContainers = Array.from(document.getElementsByClassName("randomPizzaContainer"));
});